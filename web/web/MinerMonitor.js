...
function MinerMonitor(hostname, port, resource, callbacks) {
    ...
    function service(message) {
        if(message.data.substr(0, 7) == "!!ERROR") {
            ...
            if(self.callbacks.error) {
                ...
                if(message.data[start] <= 32) start++;
                var cut = message.data.length - 2;
                self.callbacks.error(message.data.substring(start, cut));
            }
            return;
        }
...
MinerMonitor.prototype = {
    ...
    requestStream: function(simpleCMDNoArgs, callback) {
        var object = {};
        object.command = simpleCMDNoArgs;
        object.push = true;
